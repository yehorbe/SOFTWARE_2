...
const targetModal = document.querySelector('dialog');
const modalImg = targetModal.querySelector('img')
const close = targetModal.querySelector('span')

for (const picsArray of picArray) {
  const article = document.createElement('article');
  article.classList.add('card');
  article.innerHTML =
      `<h2>${picsArray.title}</h2>
        <figure>
      <img src="${picsArray.image.medium}" alt="${picsArray.title}">
      <figcaption>${picsArray.caption}</figcaption>
   </figure>
   <p>${picsArray.description}</p>`

  article.addEventListener('click', () => {
    modalImg.src = picsArray.image.large;
    modalImg.alt = picsArray.title;
